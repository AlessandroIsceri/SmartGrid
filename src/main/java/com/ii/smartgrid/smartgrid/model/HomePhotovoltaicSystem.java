...
public class HomePhotovoltaicSystem{
    ...
    public double getHourlyProduction(WeatherStatus curWeather, int curTurn) {
        ...
        double radiantLatitude = Math.toRadians(latitude);        


        double declination_angle = 23.45 * Math.sin( (360/365) * (dayOfTheYear - 81));
        double zenithAngle = radiantLatitude - declination_angle;
        ...
        double cosAzimuth = Math.sin(zenithAngle) * Math.sin(radiantLatitude) - Math.sin(declination_angle) / (Math.cos(zenithAngle) * Math.cos(radiantLatitude));
